...
def delegate_to_worker(task_name, *args, **kwargs):
    ...
    try:
        logger.info(f"📤 WORKER-DELEGATION: Sende Task '{task_name}' an Worker mit args={args[:30] if args else []} und kwargs={kwargs}")
...
def initiate_processing(session_id, upload_id):
    ...
    try:
        ...
        logger.error(f"Fehler beim Initiieren der Verarbeitung: {str(e)}")
...
def update_processing_status(session_id, status):
    ...
    try:
        ...
        logger.info(f"Verarbeitungsstatus für Session {session_id} auf '{status}' gesetzt")
        return True
    except Exception as e:
        logger.error(f"Fehler beim Aktualisieren des Verarbeitungsstatus für Session {session_id}: {str(e)}")
        return False
...
def process_upload(session_id):
    ...
    try:
        ...
        logger.error(f"Fehler bei der Verarbeitung von Session {session_id}: {str(e)}")
...
def calculate_upload_progress(session_id):
    ...
    try:
        ...
        logger.error(f"Fehler beim Berechnen des Fortschritts für Session {session_id}: {str(e)}")
        return 0
...
def estimate_remaining_time(session_id):
    ...
    try:
        ...
        logger.error(f"Fehler beim Schätzen der verbleibenden Zeit für Session {session_id}: {str(e)}")
        return None
...
def retry_processing(session_id):
    ...
    try:
        ...
        logger.error(f"Fehler beim Neustart der Verarbeitung für Session {session_id}: {str(e)}")
