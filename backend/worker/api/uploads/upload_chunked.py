...
def save_chunk(session_id, chunk_index, chunk_data):
    ...
    try:
        ...
        logger.info(f"Chunk {chunk_index} für Session {session_id} gespeichert")
        return True
    except Exception as e:
        logger.error(f"Fehler beim Speichern von Chunk {chunk_index} für Session {session_id}: {str(e)}")
        return False
...
def combine_chunks(session_id):
    ...
    try:
        ...
        logger.error(f"Fehler beim Kombinieren der Chunks für Session {session_id}: {str(e)}")
        return None
...
def cleanup_chunks(session_id):
    ...
    try:
        ...
        logger.info(f"Chunks für Session {session_id} gelöscht")
        return True
    except Exception as e:
        logger.error(f"Fehler beim Löschen der Chunks für Session {session_id}: {str(e)}")
        return False
...
def upload_chunk():
    ...
    try:
        ...
        logger.error(f"Unerwarteter Fehler beim Chunk-Upload: {str(e)}")
        return create_error_response(
            "Unerwarteter Fehler", 
            "UNEXPECTED_ERROR", 
            {"detail": str(e)}
...
def get_upload_progress(session_id):
    ...
    try:
        ...
        logger.error(f"Fehler beim Abrufen des Fortschritts für Session {session_id}: {str(e)}")
        return jsonify({
            "success": False,
