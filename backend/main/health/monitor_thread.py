...
from .base import get_health_status, set_health_status, get_api_requests_per_minute
from .connections import check_db_connection, check_redis_connection, check_celery_connection, store_health_in_redis
from .resources import get_memory_usage, get_cpu_usage, get_open_file_descriptors
    ...
    while not _stop_event.is_set():
        try:
            ...
            if int(time.time()) % 300 < 10:
                logger.info(f"Health-Monitor-Heartbeat: {health_data['status'] if health_data else 'unknown'}")
