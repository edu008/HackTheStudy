...
def apply_patches():
    ...
    for patch_func in patches:
        try:
            ...
            logger.warning("Fehler beim Anwenden von Patch %s: %s", patch_func.__name__, str(e))
...
def patch_gevent():
    ...
    if os.environ.get('USE_GEVENT', 'true').lower() not in ('false', '0', 'no'):
        try:
            ...
            logger.error("Fehler beim Gevent-Monkey-Patching: %s", str(e))
    else:
...
def patch_ssl():
    ...
    try:
        ...
        logger.error("Fehler beim Patchen von SSL: %s", str(e))
...
def patch_requests_timeout():
    ...
    try:
        ...
        logger.info(
            f"Requests-Timeout-Patch angewendet (Standard-Timeout: {os.environ.get('DEFAULT_REQUEST_TIMEOUT', '30')}s)")
    except ImportError:
        logger.debug("Requests-Modul nicht verfügbar, Timeout-Patch übersprungen")
    except Exception as e:
        logger.error("Fehler beim Patchen von Requests-Timeout: %s", str(e))
...
def patch_socket_timeout():
    ...
    try:
        ...
        logger.info(
            f"Socket-Timeout-Patch angewendet (Standard-Timeout: {os.environ.get('DEFAULT_SOCKET_TIMEOUT', '30')}s)")
    except Exception as e:
