...
from api.text_processing import detect_language
...
def detect_language_wrapper(text):
    ...
    try:
        ...
        logger.warning(f"Fehler bei der Spracherkennung: {str(e)}. Verwende Standardwert 'en'.")
        return 'en'
...
def extract_terms_from_content(content, max_terms=50):
    ...
    try:
        ...
        logger.error(f"Fehler beim Extrahieren von Begriffen: {str(e)}")
        return []
...
def parse_study_settings(settings_str):
    ...
    try:
        ...
        logger.error(f"Fehler beim Parsen der Studieneinstellungen: {str(e)}")
        return default_settings
