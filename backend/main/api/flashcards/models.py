...
from core.models import Flashcard, Topic, Upload, UserActivity, db
...
def get_flashcards(session_id=None, upload_id=None, flashcard_id=None, limit=None):
    ...
    try:
        ...
        logger.error("Fehler beim Abrufen von Flashcards: %s", str(e))
        return []
...
def save_flashcard(upload_id, front, back, category=None, difficulty=None):
    ...
    try:
        ...
        db.session.rollback()
        logger.error("Fehler beim Speichern der Lernkarte: %s", str(e))
        return None
...
def update_flashcard_statistics(flashcard_id, is_correct, difficulty=None):
    ...
    try:
        ...
        db.session.rollback()
        logger.error("Fehler beim Aktualisieren der Lernkartenstatistik: %s", str(e))
        return None
...
def get_flashcards_by_category(session_id, category):
    ...
    try:
        ...
        logger.error("Fehler beim Abrufen von Flashcards nach Kategorie: %s", str(e))
        return []
...
def get_flashcard_categories(session_id):
    ...
    try:
        ...
        categories = db.session.query(Flashcard.category)\
            ...
            .filter(Flashcard.category is not None)\
            .distinct()\
            .all()
        ...
        logger.error("Fehler beim Abrufen der Lernkartenkategorien: %s", str(e))
        return []
