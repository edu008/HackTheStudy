...
from core.models import (Connection, Flashcard, Question, Topic, Upload, User,
                         db)
...
from ..utils.session_utils import check_and_manage_user_sessions
...
def upload_file():
    ...
    try:
        ...
        logger.info("Datei-Upload-Anfrage empfangen: %s (User-ID: %s)", file.filename, user_id)
        ...
        try:
            ...
            logger.error("Fehler beim Verarbeiten der Datei: %s", str(e))
            return create_error_response(
        ...
        logger.error("Unerwarteter Fehler beim Upload: %s", str(e))
        return create_error_response(
            "Unerwarteter Fehler",
            "UNEXPECTED_ERROR",
            {"detail": str(e)}
...
def get_results(session_id):
    ...
    try:
        ...
        logger.error("Fehler beim Abrufen der Ergebnisse für Session %s: %s", session_id, str(e))
        return jsonify({
            "success": False,
