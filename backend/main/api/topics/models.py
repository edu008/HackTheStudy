...
def create_topic(upload_id, name, is_main_topic=False, parent_id=None):
    ...
    logger.info(f"Created new topic: {name} with ID: {topic.id}")
    return topic
...
def create_connection(upload_id, source_id, target_id, label):
    ...
    if existing:
        logger.info(f"Connection between {source_id} and {target_id} already exists, updating label")
    ...
    logger.info(f"Created new connection: {source_id} -> {target_id} with label: {label}")
    return connection
...
def create_connections_from_list(upload_id, connections_list):
    ...
    for conn_info in connections_list:
        ...
        if not source or not target:
            logger.warning(f"Could not create connection: source or target not found - {source_text} -> {target_text}")
            continue
