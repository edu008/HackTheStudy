...
def add_credits(user_id, credit_amount, reason=None):
    """
    Fügt einem Benutzer Credits hinzu.

    Args:
        user_id (str): Die ID des Benutzers
        credit_amount (int): Die Anzahl der hinzuzufügenden Credits
    ...
    try:
        ...
        logger.info(
            "Credits hinzugefügt: %s für Benutzer %s (%s)", 
            credit_amount, user_id, reason if reason else 'Keine Angabe'
        )

        return True
    except Exception as e:
        db.session.rollback()
        logger.error("Fehler beim Hinzufügen von Credits: %s", str(e))
        return False


def deduct_credits(user_id, credit_amount, reason=None):
    """
    Zieht einem Benutzer Credits ab.

    Args:
        user_id (str): Die ID des Benutzers
        credit_amount (float): Die Anzahl der abzuziehenden Credits
    ...
    try:
        if user.credits < credit_amount:
            logger.warning(
                "Nicht genügend Credits: Benutzer %s hat %s, benötigt %s",
                user_id, user.credits, credit_amount
            )
            return False
        ...
        # Logge den Abzug
        logger.info(
            "Credits abgezogen: %s von Benutzer %s (%s)",
            credit_amount, user_id, reason if reason else 'Keine Angabe'
        )
...
def track_token_usage(user_id, session_id, model, input_tokens, output_tokens,
                      endpoint, function_name=None, cached=False):
    """
    ...
    if not cached and user_id:
        message = f"Token-Nutzung ({model}, {input_tokens}/{output_tokens})"
        deduct_credits(user_id, credit_cost, message)
    ...
    try:
        ...
        db.session.rollback()
        logger.error("Fehler beim Speichern der Token-Nutzung: %s", str(e))
