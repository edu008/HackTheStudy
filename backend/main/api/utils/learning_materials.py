...
def generate_additional_flashcards(text, client, analysis, existing_flashcards=None, num_to_generate=5,
                                   language='en', session_id=None, function_name="generate_additional_flashcards"):
    """
    ...
    try:
        ...
        try:
            ...
            logger.error("Fehler beim Parsen der Flashcards: %s", str(e))
            ...
            try:
                ...
                logger.error("Alternativer Parsing-Versuch fehlgeschlagen: %s", str(e2))
        ...
        logger.error("Fehler bei der Generierung von Flashcards: %s", str(e))
...
def generate_additional_questions(text, client, analysis, existing_questions=None, num_to_generate=3,
                                  language='en', session_id=None, function_name="generate_additional_questions"):
    """
    ...
    try:
        ...
        try:
            ...
            logger.error("Fehler beim Parsen der Fragen: %s", str(e))
            logger.debug("AI-Antwort war: %s", response)
            ...
            return [
                {
                    'question': (f"Erkläre das Konzept von {main_topic}." 
                                if language == 'de' 
                                else f"Explain the concept of {main_topic}."),
                    'answer': ("Konnte keine Antwort generieren" 
                              if language == 'de' 
                              else "Could not generate answer"),
                    'difficulty': 'medium',
                    'topic': main_topic
                }
            ]
    except Exception as e:
        logger.error("Fehler bei der Generierung von Fragen: %s", str(e))

        # Generiere eine Standard-Frage
        default_question = (f"Erkläre das Konzept von {main_topic}." 
                           if language == 'de' 
                           else f"Explain the concept of {main_topic}.")
        default_answer = ("Fehler bei der Generierung" 
                         if language == 'de' 
...
def generate_quiz(text, client, topics=None, num_questions=5, language='en', session_id=None):
    ...
    try:
        ...
        try:
            ...
            logger.error("Fehler beim Parsen der Quizfragen: %s", str(e))

            # Fallback: Generiere eine einfache Quizfrage
            return [{
                'question': ("Was ist das Hauptthema des Textes?" 
                            if language == 'de' 
                            else "What is the main topic of the text?"),
                'options': [
                    "Option A",
                    "Option B",
                    "Option C",
                ...
                'correct_answer': 0,
                'explanation': ("Fehler bei der Generierung" 
                               if language == 'de' 
                               else "Error during generation")
            }]
    except Exception as e:
        logger.error("Fehler bei der Generierung des Quizzes: %s", str(e))
