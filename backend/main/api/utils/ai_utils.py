...
import logging
import random
import re  # Hinzugefügter Import für regex
import time
...
def query_chatgpt(prompt, client, system_content=None, temperature=0.7, max_retries=5,
                  use_cache=True, session_id=None, function_name="query_chatgpt"):
    """
    ...
    try:
        ...
        try:
            ...
            # Fallback, wenn der Import fehlschlägt
            from backend.main.core.openai_integration import OpenAIIntegration
        ...
        logger.error("Fehler in query_chatgpt: %s", str(e))
...
def _query_chatgpt_legacy(prompt, client, system_content=None, temperature=0.7, max_retries=5,
                          use_cache=True, session_id=None, function_name="query_chatgpt"):
    """
    ...
    try:
        ...
        if use_cache:
            # Nur innerhalb der use_cache-Bedingung die globale Variable deklarieren
            global _response_cache
            cache_key = f"{hashlib.md5((prompt + (system_content or '')).encode()).hexdigest()}"

            # Prüfe, ob die Antwort bereits im Cache ist
            if cache_key in _response_cache:
                logger.info("Cache-Treffer für Prompt in %s", function_name)
        ...
        if session_id:
            try:
                ...
                from backend.main.core.models import Upload
        ...
        if user_id and not check_credits_available(user_id, token_count):
            logger.warning("Nicht genügend Kredite für Benutzer %s in %s", user_id, function_name)
        ...
        while retries <= max_retries:
            try:
                ...
                retries += 1
                logger.warning(
                    "Fehler bei OpenAI-Anfrage in %s (Versuch %s/%s): %s", 
                    function_name, retries, max_retries, str(e))

                if retries <= max_retries:
                    # Exponential Backoff für Wiederholungsversuche
                    sleep_time = backoff_time + random.uniform(0, 1)
                    logger.info("Warte %.2fs vor dem nächsten Versuch...", sleep_time)
                    time.sleep(sleep_time)
                    backoff_time *= 2  # Verdopple die Wartezeit für den nächsten Versuch
                else:
                    logger.error("Alle Wiederholungsversuche fehlgeschlagen in %s: %s", function_name, str(e))
                    return f"ERROR: Konnte keine Antwort von OpenAI erhalten nach {max_retries} Versuchen."

    except Exception as e:
        logger.error("Unerwarteter Fehler in _query_chatgpt_legacy: %s", str(e))
...
def cached_query_chatgpt(prompt, system_content=None, temperature=0.7):
    ...
    try:
        ...
        logger.error("Fehler in cached_query_chatgpt: %s", str(e))
        return f"ERROR: {str(e)}"
...
def format_json_response(response, default_value=None):
    ...
    try:
        ...
        logger.warning("Konnte JSON nicht aus Antwort extrahieren: %s", str(e))
