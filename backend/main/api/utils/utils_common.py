...
def store_in_redis(key, value, expiration=86400):
    ...
    try:
        ...
        logger.error("Fehler beim Speichern in Redis: %s", str(e))
        return False
    ...
    try:
        ...
        if as_json:
            try:
                ...
                logger.warning("Konnte Wert für Schlüssel %s nicht als JSON parsen", key)
                return value

        return value
    except Exception as e:
        logger.error("Fehler beim Abrufen aus Redis: %s", str(e))
        return None
