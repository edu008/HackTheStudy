...
from .text_processing import detect_language, clean_text_for_database
from core.models import db, Upload, Topic, Connection, Flashcard, Question
...
def analyze_content(text, client, language='en', session_id=None, function_name="analyze_content"):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Fehler beim Parsen des JSON aus der AI-Antwort: {str(json_err)}")
            ...
            try:
                ...
                logger.error(f"Alternativer JSON-Parsing-Versuch fehlgeschlagen: {str(e)}")
        ...
        logger.error(f"Fehler bei der Inhaltsanalyse: {str(e)}")
        return {
            "main_topic": "Error",
            "subtopics": ["Error processing content"],
            "key_concepts": [],
...
def generate_concept_map_suggestions(text, client, main_topic, parent_subtopics, language='en', analysis_data=None):
    ...
    try:
        ...
        logger.error(f"Fehler beim Generieren der Concept Map: {str(e)}")
...
def unified_content_processing(text, client, file_names=None, user_id=None, language=None, max_retries=3, session_id=None):
    ...
    try:
        ...
        logger.info(f"Beginne Inhaltsanalyse für Session {session_id}")
        ...
        logger.info(f"Generiere Concept Map für Session {session_id}")
        ...
        from .learning_materials import generate_additional_questions, generate_additional_flashcards
        ...
        if session_id:
            logger.info(f"Speichere Ergebnisse in der Datenbank für Session {session_id}")
            try:
                # Hole den Upload
                upload = Upload.query.filter_by(session_id=session_id).first()
                
                if not upload:
                    logger.warning(f"Kein Upload für Session {session_id} gefunden")
                else:
                ...
                logger.error(f"Fehler beim Speichern der Ergebnisse in der Datenbank: {str(db_err)}")
                db.session.rollback()
        ...
        logger.error(f"Fehler bei der einheitlichen Inhaltsverarbeitung: {str(e)}")
