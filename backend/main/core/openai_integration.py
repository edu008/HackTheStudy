...
class OpenAICache:
    ...
    def __init__(self, ttl: int = CACHE_TTL):
        ...
        logger.info(f"OpenAI-Cache initialisiert. TTL: {ttl} Sekunden. Aktiviert: {self.enabled}")
    ...
    def clear(self, pattern: str = "openai:chat:*") -> int:
        ...
        try:
            ...
            if keys:
                ...
                logger.info(f"{count} Cache-Einträge gelöscht (Muster: {pattern})")
            
            return count
        except Exception as e:
            logger.error(f"Fehler beim Löschen der Cache-Einträge: {str(e)}")
            return 0
...
def count_tokens(text_or_messages, model: str = DEFAULT_MODEL) -> int:
    ...
    try:
        ...
        logger.debug(f"Tiktoken-Zählung: {token_count} Tokens für {len(text)} Zeichen")
        return token_count
    except Exception as e:
...
def check_credits_available(cost: int, user_id: Optional[str] = None) -> bool:
    ...
    try:
        ...
        if not user:
            logger.warning(f"Benutzer {user_id} nicht gefunden für Credits-Prüfung")
            return False
        ...
        logger.error(f"Fehler bei der Credits-Prüfung: {str(e)}")
        return False
...
def get_user_credits(user_id: str) -> int:
    ...
    try:
        user = User.query.get(user_id)
        if not user:
            logger.warning(f"Benutzer {user_id} nicht gefunden für Credits-Abfrage")
            return 0
        
        return user.credits
    except Exception as e:
        logger.error(f"Fehler bei der Credits-Abfrage: {str(e)}")
        return 0
...
def chat_completion(model: str, messages: List[Dict], user_id: Optional[str] = None,
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Fehler beim Tracking der fehlgeschlagenen Anfrage: {track_error}")
...
def extract_content_from_response(response: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.error(f"Fehler beim Extrahieren des Inhalts: {str(e)}")
        return ""
