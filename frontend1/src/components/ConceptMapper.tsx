...
const ConceptMapper = ({ sessionId: propsSessionId, conceptMapData }: ConceptMapperProps) => {
  ...
  const fetchInitialConcepts = async (sessionId: string) => {
    ...
    // Wenn konzeptMap-Daten bereits aus den Props verfügbar sind, nutze diese
    if (conceptMapData && conceptMapData.topics) {
    ...
    // Wenn keine Props vorhanden sind, erstelle ein leeres Hauptthema
    console.log('DEBUG: No conceptMapData available, creating empty main topic');
    
    // Erstelle ein leeres Hauptthema mit zentrierter Position
    ...
    toast({
      title: "Leere Concept Map erstellt",
  ...
  const handleAddConcept = () => {
    ...
    // Neues Konzept erstellen, mit parent_id wenn ein Hauptthema vorhanden ist
    const newConcept = {
      id: newId,
      text: newConceptText,
      x,
      y,
      // Wenn ein Hauptthema existiert, setze es als Eltern-ID
      parent_id: mainTopic ? mainTopic.id : undefined
    };

    setConcepts([...concepts, newConcept]);
    
    // Generiere Verbindung basierend auf parent_id
    if (mainTopic) {
    ...
    try {
      ...
      if (data.concept_map?.nodes && data.concept_map?.edges) {
        ...
        const otherConcepts: Concept[] = otherNodes.map(node => ({
          ...
          parent_id: node.parent_id || mainTopic.id, // Setze parent_id aus dem Knoten oder als Fallback das Hauptthema
        ...
        // Erstelle Verbindungen basierend auf parent_id der Konzepte
        const frontendConnections: Connection[] = otherConcepts.map(concept => ({
          id: `conn_${concept.id}`,
          sourceId: concept.parent_id || mainConcept.id,
          targetId: concept.id,
          label: "has subtopic" // Standardlabel
        }));
        ...
        allSubtopics.forEach((topic, index) => {
          ...
          subtopicConcepts.push({
            ...
            text: topic.name,
            parent_id: topic.parent_id || mainTopic.id, // Setze parent_id
            x,
            y,
          });
        });
        
        // Verbindungen basierend auf parent_id generieren
        const frontendConnections: Connection[] = subtopicConcepts.map(concept => ({
          id: `conn_${concept.id}`,
          sourceId: concept.parent_id || mainConcept.id,
          targetId: concept.id,
          label: "has subtopic" // Standardlabel oder aus description
        }));
